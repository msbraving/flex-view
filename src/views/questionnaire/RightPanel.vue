                    ...
                    <template v-if="['el-cascader', 'el-table'].includes(activeData.__config__.tag)">
                        ...
                        <template v-if="activeData.__config__.dataType === 'dynamic'">
                            <el-form-item label="接口地址">
                                <el-input v-model="activeData.__config__.url" :title="activeData.__config__.url" placeholder="请输入接口地址" clearable
                        ...
                        <!-- 级联选择静态树 -->
                        <el-tree v-if="activeData.__config__.dataType === 'static'" draggable :data="activeData.options" node-key="id" :expand-on-click-node="false"
...
import TreeNodeDialog from '@/views/questionnaire/TreeNodeDialog'
import IconsDialog from './IconsDialog'
import { isNumberStr } from '@/utils/index'  //正则校验匹配 0-9 开头的数字
import { inputComponents, selectComponents } from './configData/config'  // 组件类型下拉框
import { saveFormConf } from '@/utils/db'  // 保存表单属性到缓存
...
export default {
    ...
    methods: {
        ...
        addSelectItem() { //多选单选下拉添加选项
            this.activeData.__slot__.options.push({
                label: '',
                value: ''
            })
        },
        addTreeItem() { //级连选择器添加选项
            ++this.idGlobal
            this.dialogVisible = true
            this.currentNode = this.activeData.options
        },
        renderContent(h, { node, data, store }) {  // 级联选择 树节点的内容区的渲染 Function
            return (
        ...
        onDefaultValueInput(str) {   // 设置默认值输入框改变时候触发
